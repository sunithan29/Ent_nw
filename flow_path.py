...
def packetsToFlows(packets, timegap):
    try:
        ...
        print("Interrupted while extracting flows.")
...
def writeFlowsToFile(flow, filename, mode):
    outfile = open(filename, mode)
    outfile.write(flow.ip1 + ',' + flow.ip2 + ',' + str(flow.n_packet1) + ',' + str(flow.n_byte1) + ',' +
                  '%.6f' % flow.t_start1 + ',' + '%.6f' % flow.t_end1 + ',' +
                  '%.6f' % flow.getInterArrivaltime1() + ',' +
                  str(flow.n_packet2) + ',' + str(flow.n_byte2) + ',' +
                  '%.6f' % flow.t_start2 + ',' + '%.6f' % flow.t_end2 + ',' +
                  '%.6f' % flow.getInterArrivaltime2() + ',' +
                  str(flow.proto) + ',' +
                  str(flow.failedLogin) + '\n')
    outfile.close()
...
class Flow:
    ...
    def get_flow_param(self):
        param = [str(self.n_packet1), str(self.n_byte1), str(self.getInterArrivaltime1()),
                 str(self.n_packet2), str(self.n_byte2), str(self.getInterArrivaltime2()),
                 str(self.proto), str(self.failedLogin)]
        return param

    def printFlow(self):
        print(self.ip1 + ',' + self.ip2 + ',' + str(self.n_packet1) + ',' + str(self.n_byte1) + ',' +
              '%.6f' % self.t_start1 + ',' + '%.6f' % self.t_end1 + ',' +
              '%.6f' % self.getInterArrivaltime1() + ',' +
              str(self.n_packet2) + ',' + str(self.n_byte2) + ',' +
              '%.6f' % self.t_start2 + ',' + '%.6f' % self.t_end2 + ',' +
              '%.6f' % self.getInterArrivaltime2() + ',' +
              str(self.proto) + ',' +
              str(self.failedLogin) +'\n')
    ...
    def addFlow(self, flow):
        ...
        self.failedLogin = flow.failedLogin
    ...
    def addPacket(self, packet):
        if packet.source == self.ip1 and packet.dest == self.ip2:
            ...
            if self.n_packet1 == 0:
                ...
                #self.failedLogin = 0
                #self.flags.append(packet.flags)
                return
    ...
    def getInterArrivaltime1(self):
        ...
        if len(self.t_interarrival1) > 0:
            return self.t_interarrival1[floor(len(self.t_interarrival1) / 2)]
        return 0

    def getInterArrivaltime2(self):
        self.t_interarrival2.sort()
        if len(self.t_interarrival2) > 0:
            return self.t_interarrival2[floor(len(self.t_interarrival2) / 2)]
        return 0
